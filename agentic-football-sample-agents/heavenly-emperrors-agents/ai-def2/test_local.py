"""Local test for the DEF2 (Memory + Gateway) agent (player 2) — tests state summary, parsing, and fallback."""
...
def test_fallback_with_ball():
    """Test fallback when DEF2 has the ball — should PASS to nearest non-GK teammate."""
...
def test_fallback_opponent_near_goal():
    """Test that DEF2 marks opponent close to our goal."""
...
def test_parse():
    print("=== PARSE TESTS ===")
    tests = [
        ('[{"commandType":"MARK","playerId":2,"parameters":{"target_player_id":4,"tightness":"TIGHT"},"duration":5}]', 1),
        ('Here:\n[{"commandType":"PASS","playerId":2,"parameters":{"target_player_id":3,"type":"GROUND"},"duration":0}]\nDone!', 1),
        ("invalid json", 0),
        ('[]', 0),
    ]
    all_pass = True
